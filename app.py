...
# --- FUNCIÓN AUXILIAR PARA GRÁFICOS ESTILIZADOS ---
def crear_grafico_estilizado(df_var, titulo, y_label, color_linea):
    """
    Genera un gráfico de línea altamente optimizado y estéticamente limpio.
    """
    fig = px.line(
        df_var, 
        x='Fecha_Local', 
        y='Valor', 
        color='Sensor',
        labels={'Fecha_Local': 'Fecha y Hora', 'Valor': y_label},
        template="plotly_white"  # Fondo limpio y profesional sin rejilla oscura
    )
    
    # Suavizado de curva y personalización de trazo
    fig.update_traces(
        line=dict(width=2.5, shape='spline', color=color_linea), # Líneas curvas y gruesas
        mode='lines' # Solo línea para no saturar con puntos (puedes usar 'lines+markers' si hay pocos datos)
    )
    
    # Optimización de ejes, títulos y márgenes
    fig.update_layout(
        title=dict(
            text=titulo,
            font=dict(size=16, family="Arial", color="#1e293b"),
            x=0.0, # Alineado a la izquierda para estilo moderno
        ),
        hovermode="x unified", # Muestra todos los valores al pasar el cursor sobre una fecha
        margin=dict(l=40, r=20, t=50, b=40),
        height=380,
        xaxis=dict(
            showgrid=True,
            gridcolor='#f1f5f9',
            tickformat="%d %b\n%H:%M",  # Formato limpio: ej. "15 Jul 14:30"
            linecolor='#cbd5e1'
        ),
        yaxis=dict(
            showgrid=True,
                ...
                if not hydros_df.empty:
                    ...
                    with col1:
                        ...
                        if not sub_depth.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_depth, "Nivel de Agua", f"Profundidad ({unit})", "#0284c7")
                    ...
                    with col2:
                        ...
                        if not sub_temp.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_temp, "Temperatura del Agua", f"Temperatura ({unit})", "#f97316")
                            st.plotly_chart(fig, use_container_width=True)
                    ...
                    with col3:
                        ...
                        if not sub_ec.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_ec, "Conductividad Eléctrica", f"Conductividad ({unit})", "#10b981")
                            st.plotly_chart(fig, use_container_width=True)
                else:
                    st.info("No se encontraron datos del sensor Hydros 21.")

            # PESTAÑA 2: SENSOR 5TE (SUELO)
            with tab2:
                ...
                if not soil_df.empty:
                    ...
                    with col1:
                        ...
                        if not sub_wc.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_wc, "Contenido Volumétrico de Agua", f"Humedad ({unit})", "#1d4ed8")
                            st.plotly_chart(fig, use_container_width=True)
                    ...
                    with col2:
                        ...
                        if not sub_st.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_st, "Temperatura de Suelo", f"Temperatura ({unit})", "#ea580c")
                            st.plotly_chart(fig, use_container_width=True)
                    ...
                    with col3:
                        ...
                        if not sub_sec.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_sec, "EC Extracto Saturación", f"Salinidad ({unit})", "#059669")
                            st.plotly_chart(fig, use_container_width=True)
                else:
                ...
                if not system_df.empty:
                    ...
                    with col1:
                        ...
                        if not sub_bat.empty:
                            fig = crear_grafico_estilizado(sub_bat, "Nivel de Batería", "Porcentaje (%)", "#e11d48")
                            st.plotly_chart(fig, use_container_width=True)
                    ...
                    with col2:
                        ...
                        if not sub_pres.empty:
                            ...
                            fig = crear_grafico_estilizado(sub_pres, "Presión Atmosférica de Referencia", f"Presión ({unit})", "#475569")
                            st.plotly_chart(fig, use_container_width=True)
                else:
