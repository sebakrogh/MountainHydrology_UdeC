...
def cargar_datos_historicos():
    try:
        ...
        google_creds_str = st.secrets.get("GOOGLE_CREDS")
        sheet_id = st.secrets.get("HISTORICO_SHEETS_ID")
        
        if not google_creds_str or not sheet_id:
            st.error("Faltan configurar las credenciales GOOGLE_CREDS o HISTORICO_SHEETS_ID en los Secrets de Streamlit.")
            return pd.DataFrame()
